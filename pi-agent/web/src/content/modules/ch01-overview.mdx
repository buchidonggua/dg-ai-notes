...
把这句话翻成大白话：商业 Agent 把这些功能焊死在产品里，Pi 把它们拆下来变成可选模块。你想要 MCP？装一个 MCP 扩展就行。你想要子 Agent？派生一个新的 Pi 实例的扩展就有现成的。你想要 loop 模式（让 Agent 自己迭代到任务完成）？写一个扩展拦截 `turn_end` 事件再触发下一轮就行——本教程第 11 章会带你从零写一个。
...
> 注：扩展、技能、模板、主题、Pi 包的具体写法，本教程后续章节会专门展开。本节只是让你先建立"Pi 是可塑的、且缺什么都能自己补"这个心智。
...
> **后续规划**：第 11 章 扩展系统、第 12 章 测试模式、第 13 章 设计精华总结 等高阶主题暂未在本教程覆盖，感兴趣的读者可以查阅 [pi 官方仓库](https://github.com/earendil-works/pi) 的源码与文档。
